...
SortedTable = function(id) {
	...
	if(id)
		...
		this.init(this.findTable());
	this.prep();
	if(!id && this.findTable())
		new SortedTable();
};
...
SortedTable.prototype = {
	...
	init : function(elm) {
		...
		if(this.hasClass(this.table, 'regroup'))
			this.regroup();
		this.elements = this.body.getElementsByTagName('tr');
		// other properties
		this.allowMultiple = true;
		// set this to false to disallow multiple selection
		this.allowDeselect = true;
		// set this to false to disallow deselection
		// prepare the table
		this.parseCols();
	...
	findTable : function() {
		...
		for(var i = 0; i < elms.length; i++) {
			if(this.hasClass(elms[i], 'sorted') && !SortedTable.getSortedTable(elms[i]))
				return elms[i];
		}
		return null;
	},
		...
		for(var i = 0; i < this.elements.length; i++) {
			...
			for(var j = 0; j < tds.length; j++) {
				...
				for(var k = 0; k < headers.length; k++) {
					if(this.cols[headers[k]])
						this.cols[headers[k]].push(tds[j]);
				}
			}
		}
	},
	prep : function() {
		if(!this.table || SortedTable.getSortedTable(this.table))
			return;
		this.register();
		this.prepBody();
		this.prepHeader();
	...
	changeClass : function(elm, oldclass, newclass) {
		...
		for(var i = 0; i < c.length; i++) {
			c[i] = this.trim(c[i]);
			if(c[i] == oldclass || c[i] == newclass || c[i] == '')
				c.splice(i, 1);
		}
		c.push(newclass);
	...
	callBodyHover : function(e) {
		...
		if(!st)
			return false;
		if( typeof (st.onbodyhover) == 'function')
			st.onbodyhover(elm, e);
		var elm = st.findParent(elm, 'tr');
		if(e.type == 'mouseover')
			st.changeClass(elm, '', 'hover');
	...
	callBodyClick : function(e) {
		...
		if(!st)
			return false;
		if( typeof (st.onbodyclick) == 'function')
			st.onbodyclick(elm, e);
		var elm = st.findParent(elm, 'tr');
	...
	callBodyDblClick : function(e) {
		...
		if(!st)
			return false;
		if( typeof (st.onbodydblclick) == 'function')
			st.onbodydblclick(elm, e);
		return false;
	},
	callHeadHover : function(e) {
		...
		if(!st)
			return false;
		if( typeof (st.onheadhover) == 'function')
			st.onheadhover(elm, e);
		return false;
	},
	callHeadClick : function(e) {
		...
		if(!st)
			return false;
		if( typeof (st.onheadclick) == 'function')
			st.onheadclick(elm, e);
	...
	callHeadDblClick : function(e) {
		...
		if(!st)
			return false;
		if( typeof (st.onheaddblclick) == 'function')
			st.onheaddblclick(elm, e);
		return false;
	},
	// inited
	prepHeader : function() {
		...
		for(var i = 0; i < ths.length; i++) {
			...
			if(this.hasClass(ths[i], 'sortedplus') || this.hasClass(ths[i], 'sortedminus'))
				this.sort(ths[i]);
		}
	},
	prepBody : function() {
	...
	deselect : function(elm) {
		...
		for(var i = 0; i < this.selectedElements.length; i++) {
			if(this.selectedElements[i] == elm)
	...
	selectRange : function(elm1) {
		...
		var elm = elm0;
		if(this.selected(elm1)) {
			if(this.selected(elm0))
				this.deselect(elm0);
		} else {
			if(!this.selected(elm0))
				this.select(elm0);
	...
	cleanselect : function() {
		for(var i = 0; i < this.elements.length; i++) {
			if(this.selected(this.elements[i]))
				this.deselect(this.elements[i]);
		}
		this.selectedElements = new Array();
	},
	// sorting
	compareSmart : function(v1, v2) {
		...
		for(var i = 0; i < l; i++) {
			...
			if(!v2[i])
				v2[i] = "";
			if(!isNaN(parseFloat(v1[i])))
				v1[i] = parseFloat(v1[i]);
			if(!isNaN(parseFloat(v2[i])))
				v2[i] = parseFloat(v2[i]);
	...
	findSort : function() {
		var ths = this.head.getElementsByTagName('th');
		for(var i = 0; i < ths.length; i++) {
			if(this.hasClass(ths[i], 'sortedminus') || this.hasClass(ths[i], 'sortedplus'))
				return ths[i];
		}
		return null;
	},
	sort : function(elm, reverseonly) {
		...
		if(!reverseonly)
			col.sort(comparator);
		if(this.hasClass(elm, 'sortedminus') || reverseonly)
			col.reverse();
	...
	cleansort : function(except) {
		...
		for(var i = 0; i < ths.length; i++) {
			if(ths[i] == except)
				continue;
			if(this.hasClass(ths[i], 'sortedminus'))
				this.changeClass(ths[i], 'sortedminus', '');
			else if(this.hasClass(ths[i], 'sortedplus'))
				this.changeClass(ths[i], 'sortedplus', '');
		}
	},
	// movement
	...
	move : function(d, elm) {
		if(elm)
			...
			var m = true;
			for(var i = 0; i < this.selectedElements.length; i++) {
				if(!this.canMove(d, this.selectedElements[i]))
					m = false;
			}
			if(m) {
	...
	moverow : function(d, elm) {
		...
		if(d > 0) {
			...
			if(sibling.nextSibling)
				parent.insertBefore(elm, sibling.nextSibling);
			else
